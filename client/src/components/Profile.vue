...
const aboutMeErrors = computed(() =>
  v$.value.aboutMe.$errors.map((e) => e.$message),
);
      ...
      <v-card class="mx-auto">
        <v-row>
          <v-col>
            ...
            <v-card-text
              ><h4>Here is a little bit more about me:</h4>
              <p>{{ user.aboutMe }}</p></v-card-text
            >
          </v-col>
          <v-col>
            <v-img
              :src="user.avatar ? user.avatar : '/img-placeholder.png'"
              alt="Avatar"
              max-height="300"
              max-width="300"
            ></v-img
