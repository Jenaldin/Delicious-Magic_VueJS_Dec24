...
const isRecipeOwner = async (req, res, next) => {
  ...
  try {
    ...
    if (recipe.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ error: "Not authorized to edit/delete this recipe." });
    }
    req.recipe = recipe;
    next();
  } catch (error) {
...
const isProfileOwner = async (req, res, next) => {
  ...
  try {
    ...
    if (user._id.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ error: "Not authorized to edit/delete this profile." });
    }
    req.user = user;
    next();
  } catch (error) {
...
const isCommentOwner = async (req, res, next) => {
  ...
  try {
    ...
    if (comment.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ error: "Not authorized to edit/delete this comment." });
    }
    req.comment = comment;
    next();
  } catch (error) {
