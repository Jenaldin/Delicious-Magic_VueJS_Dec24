...
const get = async (userId) => {
  try {
    ...
    const user = await User.findById(userId)
      .populate("recipesOwned", "title _id type image createdAt")
      .populate("favorites", "title _id type image createdAt")
      .lean();
    if (!user) {
...
const edit = async (userId, payloadData) => {
  try {
    ...
    const editedUser = await User.findByIdAndUpdate(userId, payloadData, {
      runValidators: true,
    });
    if (!editedUser) {
      throw new Error("User not found.");
